...
class _HomePageState extends State<HomePage> {
  ...
  void didChangeDependencies() async {
    ...
    categories = await Provider.of<DestinationProvider>(context, listen: false)
        .getCategories();
              ...
              children: [
                ...
                ListView.separated(
                  ...
                  itemBuilder: (ctx, i) => CustomTile(
                    ...
                    subtitle:
                        Text('${categories[i].length} destinations available'),
                    ontap: () {
                  ...
                  separatorBuilder: (ctx, $) => Divider(
                      indent: 20, endIndent: 20, color: _theme.primaryColor),
                ),
              ],
            ),
          );
...
class CustomSearchDelegate extends SearchDelegate {
  ...
  Widget buildLeading(BuildContext context) {
    return IconButton(
      icon: Icon(Icons.arrow_back, color: _theme.accentColor),
  ...
  Widget buildSuggestions(BuildContext context) {
    return Center(
      child: Text('Please enter destination details'),
    );
  }
}
